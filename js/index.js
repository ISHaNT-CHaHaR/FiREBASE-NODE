...
if (el) {
   ////////IT has things in SignIN html too!   check those things.
   el.addEventListener('click', () => {
      ...
      if (email != '' && password != '') {
         var result = firebase
            .auth()
            .signInWithEmailAndPassword(email, password); ///for signing in!
   ...
   el.addEventListener('click', () => {
      ...
      if (email != '' && password != '' && cpassword != '') {
         if (password === cpassword) {
            var result = firebase
               .auth()
               .createUserWithEmailAndPassword(email, password); //////for creating NEW USer!
   ...
   f.addEventListener('click', () => {
      ...
      if (email != '') {
         auth
            ...
            .then(() => {
               /////Send email to user....then promise!
               window.alert(
                  `We have sent an Email to You, Please check and Verify  `
               );
            })
            .catch((error) => {
               var errorcode = error.code;
   ...
   enter.addEventListener('click', () => {
      ...
      if (fName != '' && sName != '' && phone != '') {
         ...
         usersRef.set(userData, (error) => {
            ////take the object to realtime database!
            if (error) {
               var errorcode = error.code;
               var errormessage = error.message;
               console.log(errorcode);
...
dbBlogs.on('value', (blogs) => {
   if (blogs.exists()) {
      ...
      blogs.forEach(function(singleBlog) {
         ...
         blogsHtml +=
            "<div class= 'jumbotron bg-light text-dark border border-dark'>";
